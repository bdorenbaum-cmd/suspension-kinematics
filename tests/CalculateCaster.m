classdef CalculateCaster < matlab.unittest.TestCase
    properties
    ...
    methods (Test)
        function verify_at_ride_height(testCase)
            caster = calculate_caster(testCase.UO_AT_0, testCase.LO_AT_0);
            testCase.assertEqual(caster, 6.58424236193041, "AbsTol", 1e-6, "Caster differs at ride height.")
        end

        function verify_in_bump(testCase)
            caster = calculate_caster(testCase.UO_IN_BUMP, testCase.LO_IN_BUMP);
            testCase.assertEqual(caster, 6.51136583224966, "AbsTol", 1e-6, "Caster differs at full bump.")
        end

        function verify_in_droop(testCase)
            caster = calculate_caster(testCase.UO_IN_DROOP, testCase.LO_IN_DROOP);
            testCase.assertEqual(caster, 6.67195528715766, "AbsTol", 1e-6, "Caster differs at full droop.")
        end
