classdef CalculateFVIC < matlab.unittest.TestCase
    properties
        P = KinematicsPoints( ...
    ...
    methods (Test)
         function verify_at_ride_height(testCase)
            result = calculate_fvic(testCase.P.FUCA, testCase.P.FUCF, testCase.P.FLCA, testCase.P.FLCF, testCase.UO_AT_0, testCase.LO_AT_0, testCase.WC_AT_0);

            testCase.assertEqual(result, [0, -54.656078715540133, 3.725298755717627], "AbsTol", 1e-6, "Point differs at full bump.")
        end

        function verify_in_bump(testCase)
            result = calculate_fvic(testCase.P.FUCA, testCase.P.FUCF, testCase.P.FLCA, testCase.P.FLCF, testCase.UO_IN_BUMP, testCase.LO_IN_BUMP, testCase.WC_IN_BUMP);
            
            testCase.assertEqual(result, [0.028801215962229, -43.819783786628243, -0.122973168284043], "AbsTol", 1e-6, "Point differs at full bump.")
        end

        function verify_in_droop(testCase)
            result = calculate_fvic(testCase.P.FUCA, testCase.P.FUCF, testCase.P.FLCA, testCase.P.FLCF, testCase.UO_IN_DROOP, testCase.LO_IN_DROOP, testCase.WC_IN_DROOP);

            testCase.assertEqual(result, [-0.037033264799447, -68.320380567879212, 9.355523286464759], "AbsTol", 1e-6, "Point differs at full droop.")
        end
    end
