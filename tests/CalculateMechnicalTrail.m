classdef CalculateMechnicalTrail < matlab.unittest.TestCase
    properties
    ...
    methods (Test)
        function verify_at_ride_height(testCase)
            mechanical_trail = calculate_mechanical_trail(testCase.UO_AT_0, testCase.LO_AT_0, testCase.WCP_AT_0);
            testCase.assertEqual(mechanical_trail, 7.16408173252335, "AbsTol", 1e-6, "Mechnical Trail differs at ride height.")
        end

        function verify_in_bump(testCase)
            mechanical_trail = calculate_mechanical_trail(testCase.UO_IN_BUMP, testCase.LO_IN_BUMP, testCase.WCP_IN_BUMP);
            testCase.assertEqual(mechanical_trail, 6.828327587375181, "AbsTol", 1e-6, "Mechnical Trail differs at full bump.")
        end

        function verify_in_droop(testCase)
            mechanical_trail = calculate_mechanical_trail(testCase.UO_IN_DROOP, testCase.LO_IN_DROOP, testCase.WCP_IN_DROOP);
            testCase.assertEqual(mechanical_trail, 7.55727027584718, "AbsTol", 1e-6, "Mechnical Trail differs at full droop.")
        end
