classdef CalculateToe < matlab.unittest.TestCase
  properties
    ...
    methods (Test)
        function verify_at_ride_height(testCase)
            result = calculate_toe(testCase.LO_AT_0, testCase.UO_AT_0, testCase.TRO_AT_0);
            testCase.assertEqual(result, 11.1927472041162, "AbsTol", 1e-6, "Toe differs at ride height.")
        end

        function verify_in_bump(testCase)
            result = calculate_toe(testCase.LO_IN_BUMP, testCase.UO_IN_BUMP, testCase.TRO_IN_BUMP);
            testCase.assertEqual(result, 11.2097167504475, "AbsTol", 1e-6, "Toe differs at full bump.")
        end

        function verify_in_droop(testCase)
            result = calculate_toe(testCase.LO_IN_DROOP, testCase.UO_IN_DROOP, testCase.TRO_IN_DROOP);
            testCase.assertEqual(result, 11.204981652637, "AbsTol", 1e-6, "Toe differs at full droop.")
        end
